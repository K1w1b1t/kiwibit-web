...
export default async function BlogPostPage({ params }: PostPageProps) {
  ...
  const articleJsonLd = {
    ...
    author: { '@type': 'Person', name: author?.realName ?? post.authorId },
        ...
        <article>
          <header>
            ...
            <div className="mt-4 flex items-center gap-4 text-xs text-[var(--text-muted)]">
              ...
              <span>Author: {author?.realName ?? post.authorId}</span>
            </div>
            <div className="mt-4 flex gap-2">
              <TrackedExternalLink href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(canonical)}`} slug={post.slug} className="rounded border border-[var(--surface-border)] px-3 py-1 text-xs">
                Share LinkedIn
              </TrackedExternalLink>
              <TrackedExternalLink href={`https://x.com/intent/tweet?url=${encodeURIComponent(canonical)}&text=${encodeURIComponent(post.title)}`} slug={post.slug} className="rounded border border-[var(--surface-border)] px-3 py-1 text-xs">
                Share X
              </TrackedExternalLink>
            </div>
          </header>

          <Image src={post.coverImage} alt={post.title} width={1200} height={620} className="mt-8 h-auto w-full rounded-2xl object-cover" />

          <div className="mt-10 space-y-5 text-[15px] leading-relaxed text-[var(--text-soft)]">
            {blocks.map((block, index) => {
              if (block.type === 'h2') return <h2 key={`${block.id}-${index}`} id={block.id} className="pt-4 text-2xl font-semibold text-[var(--text-main)]">{block.text}</h2>
              if (block.type === 'h3') return <h3 key={`${block.id}-${index}`} id={block.id} className="pt-2 text-xl font-semibold text-[var(--text-main)]">{block.text}</h3>
              if (block.type === 'ul') {
                return (
          ...
          {author ? (
            <div className="mt-12 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-5">
              ...
              <div className="mt-2 flex items-center gap-3">
                <Image src={author.avatar} alt={author.realName} width={56} height={56} className="h-14 w-14 rounded-full object-cover" />
                <div>
                  <p className="font-semibold">{author.realName}</p>
                  <p className="text-sm text-[var(--text-soft)]">{author.speciality}</p>
