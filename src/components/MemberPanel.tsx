...
import { Shield, Terminal, Award, ChevronRight } from 'lucide-react';
...
const TEAM_DATA: Record<string, Member> = {
  ghost: {
    ...
    posts: [
      { title: 'Bypassing EDR via RPC', date: '2024-10-15', summary: 'Hooking native APIs without triggering telemetry alerts in enterprise environments.' },
      { title: 'Advanced Kernel Exploit Techniques', date: '2024-08-22', summary: 'Exploring memory corruption exploits and bypassing modern mitigations.' }
    ],
    bounties: [
  ...
  kobalt: {
    ...
    posts: [
      { title: 'Quantum Resistant Protocols', date: '2024-11-02', summary: 'Implementation of NTRU-based key exchanges for low-power IoT devices.' },
      { title: 'Zero-Knowledge Proofs Analysis', date: '2024-09-14', summary: 'Examining soundness gaps in recursive SNARK implementations.' }
    ],
    bounties: [
...
const Header: React.FC<{ selectedId: string; onSelect: (id: string) => void }> = ({ selectedId, onSelect }) => (
    ...
    <div className="max-w-6xl mx-auto h-full px-8 flex items-center justify-between">
      <nav className="flex items-center gap-6">
        {Object.values(TEAM_DATA).map(member => (
          ...
          >
            <div className={`w-10 h-10 rounded-full overflow-hidden ring-2 ring-black grayscale opacity-40 group-hover:grayscale-0 group-hover:opacity-100 ${selectedId === member.id ? 'opacity-100' : ''}`}>
...
const HeroSection: React.FC<{ member: Member }> = ({ member }) => (
  <section className="pt-32 pb-16 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
    ...
    <div className="lg:col-span-5 flex justify-end">
      <img src={member.avatar} alt={member.codename} className="w-64 h-64 md:w-80 md:h-80 object-cover rounded-lg border border-gray-200 shadow-lg" />
    </div>
  </section>
);
              ...
              >
                {activeTab === 'dossier' && (
                  <div className="grid md:grid-cols-2 gap-12">
                    <div>
                      <h3 className="flex items-center gap-2 mb-2"><Terminal size={16} /> Technical Skills</h3>
                    ...
                    <div>
                      <h3 className="flex items-center gap-2 mb-2"><Award size={16} /> Certifications</h3>
                      <ul className="flex flex-wrap gap-2">
                        {member.skills.filter(s => s.category === 'certification').map(cert => (
                          <li key={cert.name} className="px-3 py-1 bg-gray-100 rounded text-xs">{cert.name}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
                ...
                {activeTab === 'writeups' && (
                  <div className="space-y-4">
                    {member.posts.map((post, idx) => (
                      <div key={idx} className="p-4 border rounded hover:shadow transition">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {member.bounties.map((b, idx) => (
                          <tr key={idx} className="hover:bg-gray-50 transition">
                            <td className="py-2">{b.target}</td>
                            <td className="py-2">
                              <span className={`px-2 py-0.5 rounded text-xs font-bold ${
                                b.severity === 'CRITICAL' ? 'bg-red-500 text-white' :
                                b.severity === 'HIGH' ? 'bg-orange-400 text-white' :
                                b.severity === 'MEDIUM' ? 'bg-yellow-300 text-black' : 'bg-gray-200 text-black'
                              }`}>
                                {b.severity}
                              </span>
                            </td>
                            <td className="py-2 text-right font-bold">{b.reward}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
